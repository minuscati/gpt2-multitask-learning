...
def parse_search_grid(search_str, fixed_value):
  """Comma-separated search list; omit or empty => single fixed value."""
  if search_str is None or str(search_str).strip() == '':
    return [float(fixed_value)]
  return [float(x.strip()) for x in str(search_str).split(',') if str(x).strip() != '']
...
def make_trial_checkpoint_path(args, dataset_name, lr, dropout, weight_decay):
  ...
  filename = (
    ...
    f"unf{unfreeze_tag}_emb{emb_tag}_wd{fmt_float(weight_decay)}.pt"
  )
...
def build_dataset_config(dataset_name, args, lr=None, dropout=None, weight_decay=None):
  ...
  run_tag = (
    f"{dataset_name}_{args.fine_tune_mode}_seed{args.seed}_"
    f"lr{fmt_float(lr)}_drop{fmt_float(dropout)}_"
    f"unf{unfreeze_tag}_emb{emb_tag}_wd{fmt_float(weight_decay)}"
  )
...
def run_hpsearch(args):
  lrs = parse_search_grid(args.search_lrs, args.lr)
  dropouts = parse_search_grid(args.search_dropouts, args.hidden_dropout_prob)
  weight_decays = parse_search_grid(args.search_weight_decays, args.weight_decay)
  datasets = ['sst', 'cfimdb'] if args.search_dataset == 'both' else [args.search_dataset]

  os.makedirs(args.search_output_dir, exist_ok=True)
  unfreeze_tag = "all" if args.unfreeze_last_n is not None and args.unfreeze_last_n < 0 else str(args.unfreeze_last_n)
  emb_tag = int(bool(args.train_embeddings))
  run_suffix = f"seed{args.seed}_unf{unfreeze_tag}_emb{emb_tag}"
  log_path = os.path.join(args.search_output_dir, f'hpsearch_trials_{run_suffix}.jsonl')
  ...
  n_trials = len(lrs) * len(dropouts) * len(weight_decays)
  print(f"Search grid: {len(lrs)} lr x {len(dropouts)} dropout x {len(weight_decays)} wd = {n_trials} trial(s)")
  print(f"  lr values: {lrs}")
  print(f"  dropout values: {dropouts}")
  print(f"  weight_decay values: {weight_decays}")

  for dataset_name in datasets:
    print(f"\n=== Running hpsearch on {dataset_name} ===")
    best_result = None
    for lr, dropout, weight_decay in itertools.product(lrs, dropouts, weight_decays):
      key = trial_key(
        ...
        args.unfreeze_last_n, args.train_embeddings, weight_decay
      )
      if key in completed_keys:
        print(f"\n[skip done] {key}")
        continue

      print(f"\n[search] dataset={dataset_name}, lr={lr}, dropout={dropout}, weight_decay={weight_decay}")
      seed_everything(args.seed)
      run_config = build_dataset_config(
        dataset_name, args, lr=lr, dropout=dropout, weight_decay=weight_decay
      )
      run_config.filepath = make_trial_checkpoint_path(
        args, dataset_name, lr, dropout, weight_decay
      )
      ...
      try:
        run_metrics = train(
          ...
          epoch_log_path=run_config.epoch_log_path
        )
        entry = {
          'timestamp': datetime.now(timezone.utc).isoformat(),
          'status': 'done',
        ...
        print(f"[trial done] dataset={dataset_name}, lr={lr}, dropout={dropout}, wd={weight_decay}, "
    ...
    if best_result is None:
      ...
      print(f"[best] {dataset_name}: lr={best_result['lr']}, dropout={best_result['dropout']}, "
            f"wd={best_result['weight_decay']}, dev_acc={best_result['best_dev_acc']:.4f}")

  output_path = os.path.join(args.search_output_dir, f'hpsearch_summary_{run_suffix}.json')
...
def get_args():
  ...
  parser.add_argument("--run_hpsearch", action='store_true',
                      help="Run grid search over lr, dropout, and weight_decay.")
  parser.add_argument("--search_lrs", type=str, default=None,
                      help="Comma-separated lrs to search. Omit to fix at --lr.")
  parser.add_argument("--search_dropouts", type=str, default=None,
                      help="Comma-separated dropouts to search. Omit to fix at --hidden_dropout_prob.")
  parser.add_argument("--search_weight_decays", type=str, default=None,
                      help="Comma-separated weight decays to search. Omit to fix at --weight_decay.")
